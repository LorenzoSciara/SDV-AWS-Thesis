...
public class DeviceSimulatorUpdater {
    ...
    private static final class DeviceSimulatorUpdateThread implements Runnable {
        ...
        private void handleArtifact(final String targetToken, final String gatewayToken,
            ...
            if (artifact.getUrls().containsKey("HTTPS")) {
                ...
                status.add(downloadUrl(artifact.getUrls().get("HTTP"), gatewayToken, targetToken,
                        artifact.getHashes().getSha1(), artifact.getSize())/*, artifact.getFilename()*/);
            }
        }
        ...
        private UpdateStatus readAndCheckDownloadUrl(final String url, final String gatewayToken,
            ...
            System.out.println("Try to download from " + url + "...");
            try (final CloseableHttpResponse response = httpclient.execute(request)) {
                ...
                overallread = getOverallRead(response, md, url); //count the inputStream bytes and save the inputStream file on download directory
            ...
            System.out.println("Download from " + url + " (" + overallread + " bytes) completed!");
            LOGGER.debug(message);
            return new UpdateStatus(ResponseStatus.SUCCESSFUL, message);
        }

        private static long getOverallRead(final CloseableHttpResponse response, final MessageDigest md, final String url)
                throws IOException {
            ...
            try (FileOutputStream outputStream = new FileOutputStream(downloadFile);
                ...
                try (BufferedInputStream bis = new BufferedInputStream(response.getEntity().getContent())) {
                    ...
                    //}
                    /*catch (IOException e){
                        System.out.println("Error in the file download. Error: " + e.toString());
                    }*/
        ...
        private static String hideTokenDetails(final String targetToken) {
            ...
            return targetToken.substring(0, 2) + "***"
                    + targetToken.substring(targetToken.length() - 2);
        }
