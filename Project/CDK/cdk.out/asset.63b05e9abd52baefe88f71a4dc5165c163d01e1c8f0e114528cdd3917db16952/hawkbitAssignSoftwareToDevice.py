...
def lambda_handler(event, context):
    try:
        ...
        print(f"Job ID: {job_id}, Job Data: {job_data}")
        ...
        url = f"http://{server_ip}:8080/rest/v1/targets/Lorenzo_Device/assignedDS"
        payload = json.dumps([
            {
                "id":  distribution_set['id'],
                "type": "forced"
            }
        ])
        headers = {
        ...
        if response.status_code != 200:
            print(f"Error in the Distribution set to Target Assignement execution!  Error: {response.status_code}")
            traceback.print_exc()
            put_job_failure(job_id, 'Function exception: ')
            return
        print("Distribution set to Target Assignement execution completed")
        put_job_success(job_id, 'Distribution set to Target Assignement completed')
