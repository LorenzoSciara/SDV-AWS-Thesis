...
class Battery:
    ...
    def acellerate(self, time):
        ...
        if self.stateOfCharge>=1:    
            ...
            self.batteryTemperature = int(50 +6 * math.sin(time))    
        else:
            self.stateOfCharge=0  
        return self
    
    def decellerate(self, time):
        stateOfCharge = round(self.stateOfCharge + (1.5 * time%10/10), 2)
        if self.stateOfCharge<100:
            self.stateOfCharge = stateOfCharge
            self.batteryTemperature = int(50 +6 * math.sin(time))
            self.energyAdded = round(self.energyAdded + (1.5 * time%10/10), 2)
        else:
