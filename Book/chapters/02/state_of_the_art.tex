...
The simultaneous efforts of major automotive companies such as Bosch, Renault and Stellantis, in collaboration with leading computer developers such as Arm, BlackBerry and AWS, have given rise to the Software Defined Vehicle concept, which they define as "the technological key that opens the field to enable vehicles to evolve throughout their lifetime: improved navigation and connectivity, real-time application updates, improved safety and sustainability, etc.". \cite{SDVRenault}, and "any vehicle that manages its own operations, adds functionality and enables new features primarily or entirely through software" \cite{blackberrySDV}.

It is evident that the Software Defined Vehicle represents the future of the automotive industry, promising an enriched and enduring user experience, coupled with the evolution of automotive technologies. This section further elucidates the current state of the industry, highlighting the benefits of this innovation, and the key players that are working in the industry as a enablers to develop the SDV technologies.
\subsection{Benefits}
\subsection{Enablers}
