...
Inoltre la il paradigma SDV porta un vantaggio dal punto di vista della filiare produttiva del software. Nello scenario odierno della produzione software possono esserci due meccanismi di sviluppo:
\begin{itemize}
    \item una modalità più tradizionalista in cui il software viene prodotto direttamente sul sistema, quindi sul processore stesso. Questa soluzione sicuramente è la più sconveniente in quanto richiederebbe un sovrautilizzo inutile di processori, quindi uno spreco di risorse e di denaro e di tempo.
    \item in alternativa gli sviluppatori si affidano a ingombranti strumenti di emulazione del sistema operativo sulla macchina host e al processo di compilazione incrociata, che utilizza un compilatore speciale per creare codice eseguibile per il sistema target. Una volta che il codice è sul sistema di sviluppo, è possibile eseguire un test finale di integrazione e convalida, ma la scalabilità è limitata al numero di piattaforme hardware fisiche. 
\end{itemize}

I flussi di lavoro tipici per lo sviluppo, l'integrazione e la convalida dei sistemi embedded sono i seguenti \cite{DevelopersWorkflow}:

Come esposto nei capitoli successivi, grazie all'utilizzo del software defined vehicle, quindi a sistemi operativi che si basano su architetture general porpouse per costruire una parità tra cloud e edge system, è possibile ridurre the embedded developers' workflow to cut many of the steps that are now no longer required as shown in the below diagram \cite{DevelopersWorkflow}:

\subsection{pinitiatives}
