...
Il terzo componente del simulatore è rappresentato dal cuore della TCU ovvero il sistema in grado di generare i dati simulati del veicolo. Questo elemento ha subito diverse modifiche nel corso dello sviluppo, in particolare in un primo momento era stato pensato come un elemento che comprendesse una interfaccia utente tramite linea di comando.
CODICE DA UBUNTU PER L'INPUT UTENTE 
In particolare come mostrato a linea 1 del codice il simulatore, una volta avviato, era in grado, in base ai comandi dati dall'utente tramite linea di comando, di aumentare fino ad un valore limite o diminuire la sua "velocità" fino ad arrestarsi.
In un secondo momento è stato deciso di aumentare il numero di dati telemetrici forniti in modo tale da poter raccogliere e analizzare sul cloud un numero di valori più realistici a discapito però di una generazione predefinita e non decisa dall'utente. Questa soluzione ha risolto il fatto che una TCU reale non presenterebbe una interfaccia grafica, ma semplicemente raccoglierebbe i dati generati dai sottosistemi presenti nel veicolo in funzione del tempo. In questo caso si arrivati a costruire 5 sottosistemi in grado di generare dati in funzione del tempo in modo più o meno realistico, poi questi dati vengono raccolti da uno script orchestrante che li aggrega per renderli pronti per essere inviati al servizio cloud in ascolto.
CODICE DELLA TCU PYTHON ATTUALE
