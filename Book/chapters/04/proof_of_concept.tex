...
Come mostrato nel codice \ref{lst:TCUOrchestrator} viene fatta una iterazione su ogni sottosistema presente nel simulatore, vengono raccolti i dati in formato Json e vengono inviati all'AWS IoT Core service tramite l'oggetto visto in precedenza \ref{lst:MQTTConnection} per il collegamento con il servizio stesso. Questo sistema permette di avere la massima modularità dei sottosistemi, quindi eventualmente di poterne aggiungere con aggiornamenti futuri, l'unico vincolo è di dover indicare nel file di Python init dei subsistem la lista dei sottosistemi presenti, quindi aggiungere eventuali nuovi sottosistemi presenti.
Andiamo ora a esamiare nel dettaglio un sottosistema d'esempio preso tra i sottosistemi creati per il progetto, valutandone le funzioni più importanti.
RIPORTARE IL CODICE DI UNO DEI SOTTOSISTEMI CON DESCRIZIONE. SOLO LE FUNZIONI PIù IMPORTANTI E NON TUTTO IL CODICE COME PER ESEMPIO LA FUNZIONE CHE METTE INSIEME TUTTI I DATI.
